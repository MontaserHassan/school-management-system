...
export class DomainService {
  ...
  addDomain(body:IDomainPayload):Observable<Domain> {
    return this.baseAPI.post(ApiConstant.ADD_DOMAIN, filterNullEntity(body)).pipe(
      map((res) => this.mapper.fromJson(Domain, res.data.domain))
    )
  }

  getDomainById(id:string):Observable<Domain> {
    return this.baseAPI.get(ApiConstant.GET_DOMAIN_BY_ID.replace('{id}',id)).pipe(
      map((res) => this.mapper.fromJson(Domain, res.data.domain))
    )
  }

  getDomains(params:{}):Observable<DomainsList> {
    return this.baseAPI.get(ApiConstant.GET_DOMAIN_LIST,{params}).pipe(
