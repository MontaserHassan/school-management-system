...
const createActivity = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { activityName, materials, skillId, room } = req.body;
        const { schoolId } = req.user;
        if (!materials) throw new CustomError(errorActivityMessage.MATERIALS_INVALID, 400, "activity");
        ...
        const materialsLowerCase = materials.map(material => material.toLowerCase());
        const newActivity = await activityService.createActivity(`s${newActivityId}`, activityName.toLowerCase(), materialsLowerCase, isSkillExisting._id, isSkillExisting.skillName, room, isSkillExisting.domainId, isSkillExisting.domainName, schoolId);
        if (!newActivity) throw new CustomError(errorActivityMessage.DOES_NOT_CREATED, 400, "none");
        const addActivityToRoom = await classRoomService.addActivity(room, { activityId: newActivity._id, activityName: newActivity.activityName, materials: newActivity.materials });
        if (!addActivityToRoom) throw new CustomError(errorClassRoomMessage.SKILLS_NOT_ADDED, 400, "none");
        await Promise.all(addActivityToRoom.students.map(async (student) => { return await studentService.addActivity(student.studentId, isSkillExisting._id, newActivity._id, newActivity.activityName, newActivity.materials,) }));
        await skillService.addNewActivity(skillId, newActivity._id, newActivity.activityName, newActivity.materials);
...
const updateActivityData = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { activityId, activityName, materials } = req.body;
        ...
        const newMaterials = !materials ? isActivityExisting.materials : materials.map(material => material.toLowerCase());
        const activity = await activityService.updateById(activityId, { activityName: newActivityName, materials: newMaterials });
