...
function createUserToken(user: any) {
    ...
    const token = jwt.sign({ id: user._id, email: user.email, secretKey: secretKey, role: user.role, code: user.code }, process.env.JWT_SECRET as string, { expiresIn: expiresInMilliseconds });
